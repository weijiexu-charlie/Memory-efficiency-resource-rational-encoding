...
data_all <- read.csv('corpus_data/SPRNSC_RT_lm_features.csv') %>%
  dplyr::select(subject, story_id, sentence_id, word_id, zone, word, has_punct, RT, word_surp_base_gpt2, word_surp_lambda0p0, 
                word_surp_lambda0p001, word_surp_lambda0p01, word_surp_lambda0p1,
                word_surp_lambda1p0, logfreq)

data <- data_all %>%
  mutate(story_id = as.numeric(story_id),
         sentence_id = as.numeric(sentence_id),
         word_id = as.numeric(word_id),
         RT = as.numeric(RT), # This is already spillover region RT due to model misalignment
         word_len = nchar(word),
...
# No additional data exclusion on RT since it's already done in the original corpus
...
# write.csv(dat.surp.cleaned, file = "../RT_analysis_all/data/SPRNSC_spillover_data.csv", row.names = FALSE)
...
write.csv(res_logRT$summary, file = "../RT_analysis_all/data/SPRNSC_spillover_logRT_deltaLL_crossval.csv", row.names = FALSE)
...
write.csv(res_allsurp_logRT$summary, file = "../RT_analysis_all/data/SPRNSC_spillover_logRT_allsurp_deltaLL_crossval.csv", row.names = FALSE)
