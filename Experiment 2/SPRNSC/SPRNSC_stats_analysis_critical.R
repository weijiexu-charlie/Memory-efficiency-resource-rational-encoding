...
data_all <- read.csv('corpus_data/SPRNSC_RT_lm_features.csv') %>%
  dplyr::select(subject, story_id, sentence_id, word_id, zone, word, has_punct, RT,
  ...
  mutate(RT_PLUS1 = RT,
         RT = lag(RT, 1)) # to correct RT misalignment in the corpus
...
write.csv(res_logRT$summary, file = "../RT_analysis_all/data/SPRNSC_critical_logRT_deltaLL_crossval.csv", row.names = FALSE)
...
write.csv(res_allsurp_logRT$summary, file = "../RT_analysis_all/data/SPRNSC_critical_logRT_allsurp_deltaLL_crossval.csv", row.names = FALSE)
