...
data_all <- read.csv('corpus_data/Provo_RT_lm_features.csv') %>%
  dplyr::select(subject, Text_ID, Sentence_Number, Word_In_Sentence_Number, Word, has_punct, Word_Length, RT_first_fix, RT_total, 
                word_surp_base_gpt2, word_surp_lambda0p0, 
                word_surp_lambda0p001, word_surp_lambda0p01, word_surp_lambda0p1,
                word_surp_lambda1p0, logfreq)

data <- data_all %>%
  mutate(subject = as.factor(subject),
         text_id = as.numeric(Text_ID),
         sentence_id = as.numeric(Sentence_Number),
         word_id = as.numeric(Word_In_Sentence_Number),
         RT_first_fix = as.numeric(RT_first_fix),
         RT_total = as.numeric(RT_total),
         word = Word,
         word_len = as.numeric(Word_Length),
  ...
  dplyr::select(subject, text_id, sentence_id, word_id, word, has_punct, word_len, RT_first_fix, RT_total, 
                word_surp_base_gpt2, word_surp_lambda0p0, 
                word_surp_lambda0p001, word_surp_lambda0p01, word_surp_lambda0p1,
                word_surp_lambda1p0, logfreq)

dat <- data %>%
  filter(RT_first_fix>=100 & RT_first_fix<=2000) %>%
  mutate(
    logRT_first_fix = log(RT_first_fix),
    logRT_total = log(RT_total)
  ) %>%
  group_by(subject, text_id, sentence_id) %>%
  mutate(
...
# write.csv(dat.surp.cleaned, file = "../RT_analysis_all/data/Provo_data.csv", row.names = FALSE)
...
res_logRT$per_fold
res_logRT$summary
write.csv(res_logRT$summary, file = "../RT_analysis_all/data/Provo_logRT_totalRT_deltaLL_crossval.csv", row.names = FALSE)

# Run all surp
res_logRT_allsurp <- run_cv_cumulative(
  data = d,
  model_pairs = model_pairs_logRT,
  subj_var = subj,  # same subject levels appear in train & test
  v = 10
)
res_logRT_allsurp$per_fold
res_logRT_allsurp$summary
write.csv(res_logRT_allsurp$summary, file = "../RT_analysis_all/data/Provo_logRT_allsurp_totalRT_deltaLL_crossval.csv", row.names = FALSE)
